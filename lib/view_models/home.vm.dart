...
class HomeViewModel extends MyBaseViewModel {
  ...
  bool canAcceptNewParcelOrder(NewOrder newOrder) {
    ...
    bool sameCity = newOrder.pickup.city.toLowerCase() ==
        LocationService.currentLocation.city.toLowerCase();
    bool sameState = newOrder.pickup.state.toLowerCase() ==
        LocationService.currentLocation?.state?.toLowerCase();
    bool sameCountry = newOrder.pickup.country.toLowerCase() ==
        LocationService.currentLocation?.country?.toLowerCase();
    return newOrder.isParcel &&
        canAcceptNewOrder &&
        (sameCity || sameCountry || sameState);
  }
